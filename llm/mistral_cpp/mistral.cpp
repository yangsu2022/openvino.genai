...
static void usage(const std::string& prog) {
    std::cout << "Usage: " << prog << " [options]\n"
        ...
        << "  --model_type            STRING      specify which model type used for inference (default: instruct, optional base) \n"
	<< "  --cache_dir             PATH        Cache directory to store model cache, default is 'llm_cache', set to '' to disable model cache\n";
}
...
int main(int argc, char* argv[]) try {
    ...
    std::cout << "Load mistral tokenizer took " << duration_ms << " ms" << std::endl;
    ...
    if (1 == convert_model) {
        ...
        std::cout << "Read mistral Model took " << duration_ms << " ms" << std::endl;
	...
	if (args.model_type == "base"){
	    prompt_text = input_text; //mistral-7b-v0.3 base model do not have prompt template
	}
	else if (args.model_type == "instruct"){
	    prompt_text = "[INST]" + input_text + "[/INST]";
	}
	else{
