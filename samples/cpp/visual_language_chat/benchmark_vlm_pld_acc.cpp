...
int main(int argc, char* argv[]) try {
    ...
    std::cout << "Test prompt lookup decoding!" << std::endl;
    ...
    for (size_t i = 0; i < num_warmup; i++)
        pipe->generate(prompt, ov::genai::generation_config(config));
    
    auto res = pipe->generate(prompt, ov::genai::generation_config(config));
    ...
    std::cout << "\nOutput token size:" << res.perf_metrics.get_num_generated_tokens() << std::endl;
